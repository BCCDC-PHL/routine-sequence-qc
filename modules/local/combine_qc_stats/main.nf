process combine_qc_stats {
  ...
  """
  printf "sample_id\\n${meta.id}\\n" > sample_id.csv
  awk -F ',' 'BEGIN {OFS=FS}; NR==1 { for (i=1; i<=NF; i++) {idx[\$i] = i} }; { print \$(idx["abundance_1_name"]), \$(idx["abundance_1_fraction_total_reads"]) }' ${species_abundance} | sed 's/abundance_1/most_abundant_species/g' > species_abundance_stats.csv
  ...
  paste -d ',' sample_id.csv species_abundance_stats.csv estimated_coverage_stats.csv sequence_quality_stats.csv > ${meta.id}_combined_qc_stats.csv
  """
}
