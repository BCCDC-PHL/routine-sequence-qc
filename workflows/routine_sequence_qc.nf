...
workflow ROUTINE_SEQUENCE_QC {
    ...
    ch_fastq = Channel.fromFilePairs(fastq_search_path, flat: true).map{ it -> [[id: it[0].split('_')[0]], it[1], it[2]] }.unique{ it -> it[0] }
    ch_kraken2_db = Channel.fromPath(params.kraken2_db)
    ch_sample_sheet = Channel.fromPath(findSampleSheet(params.instrument_type))
    ch_multiqc_config = Channel.fromPath( "${projectDir}/assets/multiqc_config_base.yaml" )
    ch_run_dir = Channel.fromPath(run_dir)
    ch_run_id = Channel.fromPath(run_dir).map{ it -> it.baseName }
    ch_kraken2_db = Channel.fromPath(params.kraken2_db)
    ...
    seqtk_fqchk_summary(seqtk_fqchk.out).map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "seqtk_fqchk_summary.csv", storeDir: "${outdir}/seqtk_fqchk_summary")

    mash_sketch(ch_fastq)
    mash_sketch_summary(mash_sketch.out).map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "mash_sketch_summary.csv", storeDir: "${outdir}/mash_sketch_summary")
    ...
    abundance_top_n.out.filter{ it[2] == 'Genus' }.map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "top_3_abundances_genus.csv", storeDir: "${outdir}/abundance_top_n")
    abundance_top_n.out.filter{ it[2] == 'Species' }.map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "top_5_abundances_species.csv", storeDir: "${outdir}/abundance_top_n")
    ...
    combine_qc_stats(abundance_top_n.out.filter{ it[2] == 'Species' }.map{ it -> [it[0], it[1]] }.join(seqtk_fqchk_summary.out.map{ it -> [it[0], it[1]] }).join(mash_sketch_summary.out)).map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "basic_qc_stats.csv", storeDir: "${outdir}/basic_qc_stats")
    ...
    emit:
    multiqc_report = multiqc.out.map{ it -> it[0] }
}
